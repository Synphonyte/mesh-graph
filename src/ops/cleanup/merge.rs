...
impl MeshGraph {
    ...
    pub fn merge_vertices_one_rings(
        ...
        for face_id in face_ids1.into_iter().chain(face_ids2) {
            let face = unwrap_or_return!(self.faces.get(face_id), "Face not found", result);
            for v_id in face.vertices(self) {
                if let Some(normals) = &self.vertex_normals {
                    vertex_normals.insert(
                        v_id,
                        *unwrap_or_return!(normals.get(v_id), "Normal not found", result),
                    );
                } else {
                    let normal = unwrap_or_return!(face.normal(self), "Normal not found", result);
        ...
        for range_pair_to_connect in range_pairs_to_connect {
            ...
            if range_pair_to_connect.closed {
                ...
                let start1_idx = *range_pair_to_connect.ranges[0].start();
                planned_faces.push(PlannedFace::new(
                    one_ring_v_ids1[start1_idx],
                    one_ring_v_ids1[start1_idx + 1],
                    one_ring_v_ids2[*range_pair_to_connect.ranges[1].start() + 1],
                ));

                let end1_idx = *range_pair_to_connect.ranges[0].end();
                planned_faces.push(PlannedFace::new(
                    one_ring_v_ids1[end1_idx - 1],
    ...
    fn find_start_indices(
        ...
        common_v_id: Option<VertexId>,
    ) -> Option<(usize, usize)> {
        if let Some(common_v_id) = common_v_id {
            let start_idx1 = one_ring_v_ids1
                .iter()
                .position(|&v| v == common_v_id)
                .unwrap();
            let start_idx2 = one_ring_v_ids2
                .iter()
                .position(|&v| v == common_v_id)
                .unwrap();

            Some((start_idx1, start_idx2))
        } else {
...
impl ConnectPair {
    ...
    fn compute_pairings(&self) -> Vec<Pairing> {
        ...
        let single_range = self.ranges[single_range_idx].clone();
        let other_range = self.ranges[1 - single_range_idx].clone();
...
impl Pairing {
    fn first_pair(&self, v_ids: [&[VertexId]; 2]) -> (VertexId, VertexId) {
        ...
        let other_v_id = v_ids[1 - self.single_range_idx][*self.other_range.start()];
    ...
    fn fill_faces(&self, v_ids1: [&[VertexId]; 2]) -> Vec<PlannedFace> {
        ...
        let other_range_idx = 1 - self.single_range_idx;
